    ...
    for turn in range(1):
        for set in range(sets):
            for process in range(processes):
                ...
                filename = '../model/TFIDF_xgb_model_process_{}_turn_{}_GT.pkl'.format(str(set + 1),
                                                                                       str(process + 1))
                ...
                filename = '../model/TFIDF_xgb_model_process_{}_turn_{}_Cleaning.pkl'.format(str(set + 1),
                                                                                             str(process + 1))
                pickle.dump(model, open(filename, 'wb'))
                ...
                filename = '../model/TFIDF_xgb_model_process_{}_turn_{}_Clean_No_Space.pkl'.format(str(set + 1),
                                                                                                   str(process + 1))
                pickle.dump(model, open(filename, 'wb'))
                ...
                predicted_fever_1 = model.predict(tfidf_test_vectors_1)
                predicted_fever_2 = model.predict(tfidf_test_vectors_2)
                filename = '../model/TFIDF_xgb_model_process_{}_turn_{}_Clean_Space.pkl'.format(str(set + 1),
                                                                                                str(process + 1))
                pickle.dump(model, open(filename, 'wb'))
                classification_score_test_1 = classification_report(df_test_1_classes, predicted_fever_1, digits=4)
                precision_test_1 = classification_score_test_1.split('\n')[-2].split()[2:-1][0]
